...
import { dreamSlugAliases, normalizeCategory } from "@/lib/routeAliases";

export { normalizeCategory } from "@/lib/routeAliases";

const categoryLinkTerms = [
  "anxiety", "fear", "transformation", "identity", "relationships",
  "hidden emotions", "inner conflict", "death", "body",
];
...
const availableCategorySlugs = new Set(
  allDreams.filter(isDreamIndexable).flatMap((dream) =>
    (dream.categories || []).map((category) => normalizeSlug(normalizeCategory(category)))
  )
);

export function hasCategoryRoute(category = "") {
  return availableCategorySlugs.has(normalizeSlug(normalizeCategory(category)));
}
...
export function linkCategories(text = "") {
  const terms = categoryLinkTerms.filter((term) =>
    availableCategorySlugs.has(normalizeSlug(normalizeCategory(term)))
  );
  if (terms.length === 0) return text;

  const regex = new RegExp(`\\b(${terms.sort((a, b) => b.length - a.length).join("|")})\\b`, "gi");
  return text.replace(regex, (term) => {
    const href = `/categories/${normalizeSlug(normalizeCategory(term))}`;
    return `<a href="${href}" class="underline underline-offset-4 hover:text-[#C6A96B] transition-colors">${term}</a>`;
  });
}
