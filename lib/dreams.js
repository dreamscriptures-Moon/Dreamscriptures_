...
function getRecurringDreamAnswer(dream = {}, dreamTitle = "") {
  const recurringInsight = (dream.behavioralInsights || []).find((item) =>
    /recurr|repeat/i.test(`${item?.title || ""} ${item?.content || ""}`)
  );

  return getFaqAnswer(
    recurringInsight?.content,
...
export function getDreamFAQItems(dream = {}, dreamTitle = "") {
  const faqTemplates = [
    ...
    },
    {
      ...
      answer: () => getRecurringDreamAnswer(dream, dreamTitle),
    },
  ];
  ...
  if (category && categorySlug) {
    itemListElement.push(
      ...
      },
      {
        ...
        item: `https://www.dreamscriptures.com/categories/${categorySlug}`,
      },
      {
        "@type": "ListItem",
        position: 4,
