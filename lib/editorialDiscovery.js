...
export function getCategoryEntry(slug) {
  const normalized = normalizeCategory(String(slug || "").replace(/-/g, " "));
  return getCategoryEntries().find((entry) => entry.key === normalized);
}

export function getCategoryDescription(category, categoryData) {
  if (categoryData?.emotionalNature) return categoryData.emotionalNature;
  const examples = category?.dreams.slice(0, 3).map((dream) => dream.title.toLowerCase());
  return `${category.title} dreams gather experiences connected by a shared theme. In this collection, that theme appears through ${examples.join(", ")}, and other symbols whose meaning changes with the dreamer's feelings and circumstances.`;
}
