...
function buildAdminReplyEmail(submission) {
  const firstName = submission.name.trim().split(/\s+/)[0] || "there";
  const response = escapeHtml(submission.admin_response).replaceAll("\n", "<br>");
  const interpretationLink = submission.interpretation_url
    ? `<p style="margin:26px 0 0;"><a href="${escapeHtml(submission.interpretation_url)}" style="display:inline-block;border-radius:999px;background:#1a1a1a;color:#ffffff;padding:12px 20px;text-decoration:none;font-weight:bold;">Read the interpretation</a></p>`
    : "";

  return `
    <!doctype html><html lang="en"><body style="margin:0;background:#f7f4ee;color:#2a2a2a;font-family:Arial,sans-serif;">
      <div style="max-width:620px;margin:0 auto;padding:32px 16px;"><div style="overflow:hidden;border:1px solid #e1dcd5;border-radius:20px;background:#ffffff;">
        <div style="padding:28px 32px;background:#1a1a1a;color:#ffffff;"><div style="font-size:12px;letter-spacing:2px;text-transform:uppercase;color:#d8c7a0;">DreamScriptures</div><h1 style="margin:10px 0 0;font-family:Georgia,serif;font-size:30px;font-weight:normal;">A thoughtful reply to your dream</h1></div>
        <div style="padding:28px 32px;font-size:15px;line-height:1.7;color:#3a3a3a;">
          <p style="margin:0 0 18px;">Hi ${escapeHtml(firstName)},</p>
          <div style="white-space:normal;">${response}</div>
          ${interpretationLink}
          <p style="margin:28px 0 0;padding-top:22px;border-top:1px solid #eee9e2;color:#756d64;">Warmly,<br>DreamScriptures</p>
        </div>
      </div></div>
    </body></html>`;
}
...
export async function sendAdminReplyEmail(submission) {
  const resendApiKey = process.env.RESEND_API_KEY;
  const from = process.env.RESEND_FROM_EMAIL;
  if (!resendApiKey || !from) throw new Error("Resend environment variables are not configured.");
  if (!submission.email) throw new Error("The submission does not have a recipient email address.");
  if (!submission.admin_response?.trim()) throw new Error("A saved response is required before sending.");

  const result = await new Resend(resendApiKey).emails.send(
    {
      from,
      to: submission.email,
      subject: "A reply from DreamScriptures",
      html: buildAdminReplyEmail(submission),
    },
    { idempotencyKey: `admin-reply/${submission.id}` }
  );
  if (result.error) throw result.error;
  return result.data;
}
