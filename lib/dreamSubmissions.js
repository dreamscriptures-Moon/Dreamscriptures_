...
import { initializeRepeatCommunityPayment } from "@/lib/payments/intents";
        ...
        <div style="max-width:620px;margin:0 auto;padding:32px 16px;">
          <div style="overflow:hidden;border:1px solid #e1dcd5;border-radius:20px;background:#ffffff;">
            <div style="padding:28px 32px;background:#1a1a1a;color:#ffffff;">
              ...
              <h1 style="margin:10px 0 0;font-family:Georgia,serif;font-size:30px;font-weight:normal;">Your dream has been received</h1>
            </div>
            <div style="padding:28px 32px;font-size:15px;line-height:1.7;color:#3a3a3a;">
              ...
              <p style="margin:0 0 18px;">Your dream has been safely received.</p>
              <p style="margin:0 0 18px;">Your reference ID is <strong>${escapeHtml(submission.referenceId)}</strong>.</p>
              <p style="margin:0 0 18px;">Every submission is personally reviewed.</p>
              <p style="margin:0 0 18px;">If your dream is selected for interpretation, we'll contact you by email and let you know when it has been published.</p>
              <p style="margin:0 0 18px;">Because we receive many dream submissions, we can't guarantee every dream will become an article, but every dream is read and appreciated.</p>
...
export async function submitDream(input) {
  ...
  try {
    ...
    if (previousCommunitySubmissions > 0) {
      ...
      return { ok: true, paymentRequired: true, ...payment };
    }

    await persistDreamSubmission(completedSubmission);
  } catch (error) {
    console.error("Failed to process dream submission:", error);
    return { ok: false, errors: {}, storageFailed: true };
  }
