...
export function getPaymentConfig() {
  ...
  const missing = [
    ...
    !process.env.PAYSTACK_CURRENCY && "PAYSTACK_CURRENCY",
    !process.env.PERSONAL_INTERPRETATION_PRICE_SUBUNIT &&
      "PERSONAL_INTERPRETATION_PRICE_SUBUNIT",
  ].filter(Boolean);
  if (missing.length) {
  ...
  return {
    ...
    paymentLinkSecret,
    currency: process.env.PAYSTACK_CURRENCY.toUpperCase(),
    repeatCommunityAmount: positiveInteger(process.env.COMMUNITY_REPEAT_PRICE_SUBUNIT, 99),
    personalAmount: positiveInteger(process.env.PERSONAL_INTERPRETATION_PRICE_SUBUNIT),
  };
}
