...
export function getGuideText(guide = {}) {
  return [
    ...
    guide.intro,
    ...(guide.content || []),
    ...(guide.sections || []).flatMap((section) => [section.title, section.body, section.table?.caption, ...(section.table?.headers || []), ...(section.table?.rows || []).flat()]),
    ...(guide.actions || []),
  ].filter(Boolean).join(" ");
}
...
export function createGuideMetadata({ slug, title, description }) {
  ...
  const rawTitle = String(title || "Dream Guide").replace(/\s+/g, " ").trim();
  const brandLength = " | DreamScriptures".length;
  let resolvedTitle = rawTitle;

  if (rawTitle.length + brandLength > 70) {
    const candidates = [
      rawTitle.match(/^[^?]+\?/)?.[0],
      rawTitle.split(":")[0],
      rawTitle.split(" | ")[0],
      rawTitle.split(" (")[0],
      rawTitle.split(/\s+Without\s+/i)[0],
    ].filter((candidate) => candidate && candidate !== rawTitle);
    const focused = candidates.find((candidate) => candidate.length >= 10 && candidate.length + brandLength <= 70);
    if (focused) {
      resolvedTitle = focused === "History & Culture" ? "Dream History & Culture" : focused;
    }
  }

  const metadataTitle = resolvedTitle.length + brandLength > 70
    ? { absolute: resolvedTitle }
    : resolvedTitle;
  return createPageMetadata({ title: metadataTitle, ogTitle: resolvedTitle, description, path, type: "article" });
}

export function createMetadataFromGuide(guide = {}) {
  const custom = guide.metadata || {};
  const generated = createGuideMetadata({ slug: guide.slug, title: custom.title || guide.seoTitle || guide.title, description: custom.description || guide.description || guide.intro });
  return {
    ...generated,
    ...custom,
    title: generated.title,
    alternates: { ...generated.alternates, ...custom.alternates, canonical: generated.alternates.canonical },
    openGraph: { ...generated.openGraph, ...custom.openGraph, url: generated.openGraph.url },
    twitter: { ...generated.twitter, ...custom.twitter },
  };
}
...
export function createGuideSchemas({ slug, title, description, faqs = [] }) {
  ...
  const schemas = [
    ...
    { "@context": "https://schema.org", "@type": "Article", headline: title, description, mainEntityOfPage: { "@type": "WebPage", "@id": url }, author: { "@type": "Person", name: "Amber Balentine", url: `${SITE_URL}/author` }, publisher: { "@type": "Organization", name: "DreamScriptures", url: SITE_URL } },
  ];
