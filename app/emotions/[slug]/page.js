...
export default async function EmotionPage({ params }) {
  ...
  return <main className="min-h-screen bg-[#f7f3ed] text-[#29251f]"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} /><SiteHeader /><article>
    <header className="border-b border-[#ded7cd] bg-[#fbf9f5]"><div className="mx-auto max-w-5xl px-6 py-12 md:py-20"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Emotions", href: "/emotions" }, { label: emotion.title }]} /><p className="text-xs uppercase tracking-[0.2em] text-[#8f743c]">Emotional dream guide</p><h1 className="mt-4 max-w-4xl font-serif text-5xl leading-[1.08] md:text-7xl">Dreams About {emotion.title}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-[#686159]">{emotion.intro}</p><div className="mt-7 max-w-3xl border-l border-[#b89b62] pl-5"><p className="text-base leading-7 text-[#686159]">You may recognize all of this feeling, or just a little of it. This page looks at ways it can appear in dreams and offers questions to help you reflect. Keep your own experience in mind; a feeling alone cannot tell you what a dream means.</p>{connectedDreams.length > 0 && <a href="#emotion-dreams" className="mt-3 inline-flex min-h-11 items-center text-sm font-medium text-[#806431] underline underline-offset-4 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#8f743c]">See dreams connected with this feeling</a>}</div></div></header>
