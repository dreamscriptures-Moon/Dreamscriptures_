import PageStartingPoints from "@/app/components/PageStartingPoints";
...
export default function EmotionsPage() {
  ...
  return <main className="min-h-screen bg-[#f7f3ed] text-[#29251f]"><SiteHeader />
    <header className="border-b border-[#ded7cd] bg-[#fbf9f5]"><div className="mx-auto max-w-6xl px-6 py-12 md:py-20"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Emotions" }]} /><div className="max-w-3xl"><p className="mb-4 text-xs uppercase tracking-[0.22em] text-[#8f743c]">Start with how it felt</p><h1 className="font-serif text-5xl leading-[1.05] md:text-7xl">Dream Emotions</h1><p className="mt-6 text-lg leading-8 text-[#686159]">Did the feeling stay with you after the dream faded? Explore dreams and reflection questions connected to that feeling, even if you can’t remember exactly what happened.</p></div></div></header>
    <div className="mx-auto max-w-6xl px-6">
      <PageStartingPoints title="Where would you like to begin?" hint="Feelings can overlap or be hard to name. Choose whatever sounds closest; these aren’t labels you need to apply to yourself." items={[
        { href: "#dream-feelings", label: "I want to start with a feeling", description: "Find a description that sounds familiar, then read the related dreams and questions." },
        { href: "/categories", label: "I remember the dream more than the feeling", description: "Look through people, places, and situations instead." },
      ]} />
      <section id="dream-feelings" className="scroll-mt-28 pb-16 pt-6 md:pb-24"><SectionHeading eyebrow="See what sounds familiar" title="Find a Feeling You Recognize" intro="Read the short descriptions and choose one that fits something you felt. It doesn’t have to describe the whole dream, and it doesn’t determine what your dream means." /><div className="mt-10 grid gap-5 md:grid-cols-2">{featured.map((emotion) => <Link key={emotion.slug} href={`/emotions/${emotion.slug}`} className="group rounded-[1.75rem] border border-[#ded7cd] bg-[#fcfaf6] p-7 transition hover:border-[#b89b62]"><div className="flex items-start justify-between gap-4"><h2 className="font-serif text-2xl group-hover:text-[#806431]">{emotion.title}</h2>{emotion.count > 0 && <span className="shrink-0 text-xs uppercase tracking-[0.14em] text-[#8f743c]">{emotion.count} dreams</span>}</div><p className="mt-3 leading-7 text-[#70685f]">{emotion.intro}</p></Link>)}</div><div className="mt-10 border-t border-[#ded7cd] pt-8"><h3 className="font-serif text-2xl">More feelings to start with</h3><div className="mt-5 flex flex-wrap gap-3">{remaining.map((emotion) => <Link key={emotion.slug} href={`/emotions/${emotion.slug}`} className="rounded-full border border-[#d9d0c4] bg-white/60 px-4 py-2 text-sm hover:border-[#b89b62]">{emotion.title}{emotion.count ? ` · ${emotion.count}` : ""}</Link>)}</div></div></section>
