...
export default async function AdminDashboard() {
  ...
  const recent = recentResult.status === "fulfilled" ? recentResult.value : { submissions: [] };
  const dataError = statsResult.status === "rejected" || recentResult.status === "rejected";
  for (const result of [statsResult, recentResult]) {
    if (result.status === "rejected") console.error("Admin dashboard data unavailable:", result.reason);
  }
  const cards = [
  ...
  return (
    <>
      ...
      {dataError && <div role="alert" className="mt-5 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">Submission data is temporarily unavailable. <a href="/admin" className="ml-2 font-semibold underline">Retry loading</a></div>}
      ...
      <section className="mt-8 rounded-xl border border-slate-200 bg-white">
        ...
        <div className="divide-y divide-slate-100">
          ...
          {recentResult.status === "fulfilled" && !recent.submissions.length && <p className="px-5 py-8 text-sm text-slate-500">No submissions yet.</p>}
        </div>
      </section>
    </>
  );
