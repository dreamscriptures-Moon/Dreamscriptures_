...
export default function CategoriesPage() {
  ...
  return <main className="min-h-screen bg-[#f7f3ed] text-[#29251f]"><SiteHeader />
    <section className="border-b border-[#ded7cd] bg-[#fbf9f5]"><div className="mx-auto max-w-6xl px-6 py-12 md:py-20"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Categories" }]} /><div className="grid items-end gap-10 md:grid-cols-[1.25fr_.75fr]"><div><p className="mb-4 text-xs uppercase tracking-[0.22em] text-[#8f743c]">Browse by what you remember</p><h1 className="font-serif text-5xl leading-[1.05] md:text-7xl">Dream Categories</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-[#686159]">Was there a person, place, or moment in your dream that stayed with you? Explore related dreams grouped by what you remember, so you can find something that feels close to your experience.</p></div><div className="rounded-3xl border border-[#ded7cd] bg-white/70 p-6"><p className="mb-4 text-sm text-[#686159]">Remember something specific? Search for it here, such as a house, a snake, or being chased.</p><SearchBar /></div></div></div></section>
    <div className="mx-auto max-w-6xl px-6">
      <PageStartingPoints title="What do you remember most?" hint="You don’t have to put your dream in the right category. More than one may fit; each is just a place to start." items={[
        { href: "#dream-themes", label: "Something that happened or appeared", description: "Browse themes, then open a dream that sounds familiar to consider possible meanings." },
        { href: "/emotions", label: "The feeling it left me with", description: "Start with the emotion if the details are harder to remember." },
      ]} />
      <section id="dream-themes" className="scroll-mt-28 pb-16 pt-6 md:pb-24"><SectionHeading eyebrow="Start with something familiar" title="Browse Dream Themes" intro="Choose a theme to see related dreams. The interpretations offer possibilities to reflect on; your own feelings and experiences still matter." /><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{categories.map((category) => { const data = categoriesData[category.key]; return <Link key={category.slug} href={`/categories/${category.slug}`} className="group rounded-[1.75rem] border border-[#ded7cd] bg-[#fcfaf6] p-6 transition hover:-translate-y-0.5 hover:border-[#b89b62] motion-reduce:transform-none"><p className="text-xs uppercase tracking-[0.16em] text-[#8f743c]">{category.count} dream{category.count === 1 ? "" : "s"}</p><h2 className="mt-3 font-serif text-2xl group-hover:text-[#806431]">{category.title} Dreams</h2><p className="mt-3 text-sm leading-6 text-[#70685f]">{data?.emotionalNature || `Dreams connected through ${category.title.toLowerCase()} imagery, experiences, and emotional context.`}</p></Link>; })}</div></section>
