...
export default async function CategoryPage({ params }) {
  ...
  return <main className="min-h-screen bg-[#f7f3ed] text-[#29251f]"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbSchema) }} /><SiteHeader />
    <article><header className="border-b border-[#ded7cd] bg-[#fbf9f5]"><div className="mx-auto max-w-5xl px-6 py-12 md:py-20"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Categories", href: "/categories" }, { label: category.title }]} /><p className="text-xs uppercase tracking-[0.2em] text-[#8f743c]">Dream category · {category.count} interpretations</p><h1 className="mt-4 max-w-4xl font-serif text-5xl leading-[1.08] md:text-7xl">{category.title} Dreams</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-[#686159]">{intro}</p><div className="mt-7 max-w-3xl border-l border-[#b89b62] pl-5"><p className="text-base leading-7 text-[#686159]">This page gathers dreams with a similar theme. Read the overview, or go straight to the dream examples and find one close to yours. The details may differ; use the possible meanings as questions to consider, rather than answers you need to accept.</p><a href="#category-dreams" className="mt-3 inline-flex min-h-11 items-center text-sm font-medium text-[#806431] underline underline-offset-4 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#8f743c]">Find a dream that sounds familiar</a></div></div></header>
      <div className="mx-auto max-w-5xl px-6">
        ...
        <section id="category-dreams" className="scroll-mt-28 py-16"><SectionHeading eyebrow="Dream dictionary" title={`Popular ${category.title} Dreams`} intro={`Read the individual interpretations that make up this ${category.title.toLowerCase()} collection.`} /><DreamPreviewGrid dreams={category.dreams} limit={8} /></section>
